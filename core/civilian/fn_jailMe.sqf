...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		hintSilent parseText format["Temps Restant:<br/> <t size='2'><t color='#FF0000'>%1</t></t>",_countDown,[life_bail_amount] call life_fnc_numberText,if(isNil "life_canpay_bail") then {"Yes"} else {"No"}]
	};
