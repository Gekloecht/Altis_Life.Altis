...
if(_veh distance impound_obj < 50) then
{
	...
	if(count _check > 0) exitWith {hint "Il y a deja une voiture sur le spawn.";};
	if(_price > life_cash) then
	{
		_price = _price + 200;
		if(_price > life_atmcash) exitWith {hint "Vous n'avez pas assez d'argent sur votre compte pour cela."};
	...
	hint format["Vous avez récupéré votre %1 pour $%3",_name,[_price] call life_fnc_numberText];
	detach _veh;
